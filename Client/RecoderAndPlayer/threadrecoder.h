#ifndef THREADRECODER_H
#define THREADRECODER_H
...
class ThreadRecoder : public QMainWindow
{
    Q_OBJECT
public:
    explicit ThreadRecoder(QWidget *parent = nullptr);
    void RecoderAudio();     //功能：录制音频
    void RecoderAudioStop(); //功能：录制音频停止
    qint64 addWavHeader(QString catheFileName, QString wavFileName);
signals:
    void RecordStopReady(); //信号：已经停止录音
    void RecordStart();     //信号：开始录音
    void Recording();       //信号：正在录音
private:
    bool m_isRecord;
    bool m_isPlay;
